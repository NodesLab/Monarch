...
package net.monarch.commands.text
...
object GenStrCommand : Command {
  ...
  override suspend fun execute(session: CommandSession) {
    ...
    if (!MathUtility.isNumber(session.arguments[0])) {
  ...
  private fun generateString(useLetters: Boolean, useUppercaseLetters: Boolean, useNumbers: Boolean, useSpecialSymbols: Boolean, length: Int): String {
    ...
    if (useLetters) { result += letters }
    if (useUppercaseLetters) { result += uppercaseLetters }
    if (useNumbers) { result += numbers }
    if (useSpecialSymbols) { result += specialSymbols }
